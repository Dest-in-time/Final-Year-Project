...
Class Action {
	...
	function login(){
		...
		$type2 = array("","admin","faculty","student");
			$qry = $this->db->query("SELECT *,concat(firstname,' ',lastname) as name FROM {$type[$login]} where email = '".$email."' and password = '".$password."'  ");
		if($qry->num_rows > 0){
		...
		if($academic->num_rows > 0){
			foreach($academic->fetch_array() as $k => $v){
				...
				if ($login == 3){
					$query1 = $this->db->query("SELECT * FROM student_list WHERE email = '".$email."' AND matric_no = '".$matric_no."';");
	...
	function save_criteria(){
		...
		$stmt = $this->db->prepare("INSERT INTO criteria_list(criteria, order_by) VALUES(?, ?)");
		$stmt->bind_param("si", $data, $lastOrder);
	...
	function save_faculty(){
		...
		$check = $this->db->query("SELECT * FROM faculty_list where faculty_id ='$faculty_id' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 3;
			exit;
		}
